...
TEST_SUITE("[PACK]") 
{
    ...
    TEST_CASE("strings and binary arrays")
    {
        std::mt19937 eng(std::random_device{}());
        std::vector<uint8_t> buf1, buf2;
        ...
        // Test deserialization
        {
            ...
            REQUIRE(n == nn);
            REQUIRE(o == oo);
            REQUIRE(num_errors(p, pp) == 0);
            REQUIRE(num_errors(q, qq) == 0);
            REQUIRE(num_errors(r, rr) == 0);
            REQUIRE(num_errors(s, ss) == 0);
            REQUIRE(num_errors(t, tt) == 0);
            REQUIRE(num_errors(u, uu) == 0);
    ...
    TEST_CASE("tuple")
    {
...

        {
            ...
            auto in = source(buf2);
            deserialize(in, aa);
            REQUIRE(std::get<0>(a) == std::get<0>(aa));
            REQUIRE(std::get<1>(a) == std::get<1>(aa));
            REQUIRE(std::get<2>(a) == std::get<2>(aa));
        }
    }
